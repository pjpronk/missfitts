...
# Set faces to "none" for transparency, keep text/lines white
plt.rcParams.update({
...
# =====================================================================
# FIGURE 1: Trials vs Completion Time
# =====================================================================
plt.figure(figsize=(12, 6))

sns.lineplot(
    data=df_all, x='Continuous_Trial', y='Time', hue='Condition', 
    palette=palette_main, linewidth=2.5, errorbar=('ci', 95), alpha=0.8
)

plt.axvline(x=60.5, color='white', linestyle='--', linewidth=1.5)
plt.text(61, df_all['Time'].max() * 0.8, ' Feedback Removed', color='white', fontsize=12)
...
sns.despine()
plt.tight_layout()
# Note: transparent=True guarantees the background is completely clear
plt.savefig("Figure1_Trials_vs_Time_Smooth.png", dpi=300, transparent=True)
plt.close()
print("Saved Figure1_Trials_vs_Time_Smooth.png")


# =====================================================================
# FIGURE 2: Push vs Pull - Change in Completion Time
# =====================================================================
...
if 3 in set_means['Set'].values and 4 in set_means['Set'].values:
    ...
    sns.pointplot(
        data=change_df, x='Condition', y='Time_Change', hue='Condition', 
        order=[label_push, label_pull], palette=palette_main, capsize=0.1, 
        errorbar=('ci', 95), join=False, markers='o', legend=False
    )
    ...
    sns.despine()
    plt.tight_layout()
    plt.savefig("Figure2_Change_in_Time.png", dpi=300, transparent=True)
    plt.close()
    print("Saved Figure2_Change_in_Time.png")
...
order_4 = [
    f"{label_push}\n(Set 1-3)", f"{label_push}\n(Set 4)",
    f"{label_pull}\n(Set 1-3)", f"{label_pull}\n(Set 4)"
]

palette_4 = {
    f"{label_push}\n(Set 1-3)": "#ff6666", f"{label_push}\n(Set 4)": "#cc0000",    
    f"{label_pull}\n(Set 1-3)": "#66b3ff", f"{label_pull}\n(Set 4)": "#005ce6"     
}

plt.figure(figsize=(10, 7))

sns.pointplot(
    data=df_all, x='Phase_Group', y='Time', hue='Phase_Group',
    order=order_4, palette=palette_4, capsize=0.1, 
    errorbar=('ci', 95), markers='o', legend=False
)
...
sns.despine()
plt.tight_layout()
plt.savefig("Figure3_Phase_Comparison.png", dpi=300, transparent=True)
plt.close()
print("Saved Figure3_Phase_Comparison.png")

print("All 3 transparent figures generated successfully!")